...
class BiFPN(nn.Module):
    def __init__(self, args, epsilon=0.0001):
        """
        Expect args to be e.g. [256, 512, 1024, 64, 2]
        which correspond to:
          args[0:3] = in_channels => [256, 512, 1024]
          args[3]   = feature_size => 64
          args[4]   = num_layers   => 2
        """
        super(BiFPN, self).__init__()
        in_channels = args[0:3]   # [256, 512, 1024]
        feature_size = args[3]    # 64
        num_layers = args[4]      # 2

        # p3, p4, p5
        self.p3 = nn.Conv2d(in_channels[0], feature_size, kernel_size=1, stride=1, padding=0)
        self.p4 = nn.Conv2d(in_channels[1], feature_size, kernel_size=1, stride=1, padding=0)
        self.p5 = nn.Conv2d(in_channels[2], feature_size, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, inputs):
        """
        Expect inputs=[c3, c4, c5].
        Returns [p3_out, p4_out, p5_out, p6_out, p7_out].
        """
        c3, c4, c5 = inputs
        p3_x = self.p3(c3)
        p4_x = self.p4(c4)
